// --- tiny PLY point-cloud parser: x,y,z + optional r,g,b ---------------------
// Supports ascii and binary_little_endian/big_endian, any property order.
//
// Prefer parsePLYAsync below; parsePLY is exported for the worker that backs it.
...
// Run the decode in a Worker, handing over the ArrayBuffer so nothing is copied.
// Sparse clouds run 30-106 MB and the parser reads every property through a
// DataView closure per value, which stalls the render loop for hundreds of
// milliseconds when done inline.
export function parsePLYAsync(buffer) {
  let worker;
  try {
