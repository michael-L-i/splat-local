...
def run_config(name: str, flags: dict[str, str], args, dataset: Path, out_dir: Path,
    ...
    # In curve mode every eval checkpoint is scored, so export at the same cadence: the
    # splat count per checkpoint is half the story (does growth stall before the steps do?).
    export_every = args.eval_every if args.curve else args.steps
    ...
    splats_by_iter = {int(re.search(r"(\d+)", p.name).group(1)): ply_splat_count(p) for p in plys}
    return {
        "config": name,
        "flags": merged,
        "cmd": cmd,
        ...
        "splats_by_iter": splats_by_iter,
        "brush_native_eval": native_evals[-1] if native_evals else None,
        "brush_evals_by_iter": {e["iter"]: e for e in native_evals},
...
def curve_rows(result: dict, gt_dir: Path, ref_res: int) -> list[dict]:
    """One scored row per eval checkpoint of a single run — the PSNR-vs-steps curve.

    Read these as a *screen*, not a verdict on short runs. Brush's schedules are functions of
    `total_train_iters`, not of absolute iteration: the mean-position LR decays at rate
    (lr_mean_end/lr_mean)^(1/total_train_iters) (brush-train/src/train.rs:122), and both refine
    and opacity decay key off iter/total_train_iters (train.rs:415, :795). So the checkpoint at
    iter N of a longer run has *not* finished annealing, while a dedicated N-step run has. That
    effect is one-directional: it understates a real N-step run.

    `growth_stop` is the part that is *not* one-directional, and it decides how far the screen
    carries:

    - Against an N-step run that keeps the parent's absolute `growth_stop`, the splat set is
      built by the same schedule and only the annealing differs, so the screen is a genuine
      lower bound. A checkpoint matching the final score proves that run would too.
    - Against an N-step run with `growth_stop` scaled to N/2, densification is cut short as
      well: fewer splats (less capacity) but proportionally more refinement. Those pull in
      opposite directions and the net is unknown, so the screen is only indicative and the
      arm has to be trained.

    Either way a checkpoint that *lags* the final score proves nothing, because the annealing
    deficit alone could account for it.
    """
    rows = []
    for eval_dir in map(Path, result["eval_dirs"]):
        iter_n = int(re.search(r"(\d+)", eval_dir.name).group(1))
        row = {k: v for k, v in result.items()
               if k not in ("eval_dirs", "splats_by_iter", "brush_evals_by_iter", "per_image")}
        row["config"] = f"{result['config']}@{iter_n}"
        row["iter"] = iter_n
        row["eval_dir"] = str(eval_dir)
        row["splats"] = result["splats_by_iter"].get(iter_n)
        row["brush_native_eval"] = result["brush_evals_by_iter"].get(iter_n)
        # The parent run's wall time and throughput say nothing about what an N-step run
        # would cost — printing them on every row would read as exactly that claim. They
        # belong to the run as a whole, so they go in the header instead.
        row["wall_s"] = row["steps_per_s"] = row["train_s"] = None
        rows.append(score(row, gt_dir, ref_res))
    return rows
...
def main() -> int:
    ...
    p.add_argument("--curve", action="store_true",
                   help="score every eval checkpoint of the run, not just the last, and export "
                        "a ply at each. Requires --eval-every. Screens where quality plateaus; "
                        "see curve_rows() for why it understates short runs.")
    ...
    if args.curve and not args.eval_every:
        p.error("--curve needs --eval-every to have more than one checkpoint to score")
    ...
    for name in names:
        ...
        if args.curve:
            run_totals.append(f"{name}: {run['train_s']:.0f}s training loop, "
                              f"{run['steps_per_s']:.1f} steps/s over {args.steps} steps")
            results.extend(curve_rows(run, gt_dir, args.ref_res))
        else:
    ...
    header = (f"held-out eval · {n_eval} views · normalised to {args.ref_res} px "
              f"long edge · {args.steps} steps")
    if args.curve:
        header += ("\ncheckpoints of one run — understates a real N-step run "
                   "(see curve_rows). " + "; ".join(run_totals))
